...
import type { GeneratedFile, WebsiteSettings } from "@/lib/projects/types"
...
export function WebsiteSettingsPanel({ projectId, initialSettings, projectName, projectFiles, databaseIntegration, githubIntegration, onSaved }: Props) {
  ...
  const runSupabaseProvisioning = async () => {
    const authHeader = await getAuthHeader()
    ...
    const json = await res.json().catch(() => ({}))
    if (!res.ok) throw new Error(json?.error || "Failed to provision Supabase.")
    return json
  }
